...
describe('RestoreContactSupportUseCase', () => {
  let useCase: RestoreContactSupportUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get(RestoreContactSupportUseCase);
  });
  ...
  it('restores a soft-deleted row, audits, invalidates cache, logs start+end', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      'RestoreContactSupportUseCase start',
      expect.objectContaining({ traceId: 'trace-id', id: ID }),
    );
    expect(logger.info).toHaveBeenCalledWith(
      'RestoreContactSupportUseCase end',
      expect.objectContaining({ traceId: 'trace-id', id: ID }),
    );
  });
