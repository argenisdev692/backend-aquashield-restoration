...
> **For CRUDs with business logic**: use `.claude/skills/ARCHITECTURE-DEFAULT/SKILL.md`.
...
> **Root tree (`src/main.ts`, `app.module.ts`, `core/`, `shared/`, `logger/`, `modules/`) is identical across DEFAULT and ENTERPRISE.** See `.claude/skills/ARCHITECTURE-DECISION-GUIDE.md` for the canonical layout — do not restate per skill. Below is only what is specific to the ENTERPRISE tier (the `modules/{module}/` Hex/DDD template).
