...
> **Pattern**: Controller → Service → Repository. No CQRS, no Use Cases, no domain events, **no cache, no audit, no exports, no bulk operations, no soft-delete visibility, no users/auth response shape**. Any of those features is an upgrade trigger to `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md`.
...
> **For CRUDs with business logic (cache + audit + exports + bulk + soft-delete visibility)**: use `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md`.
...
> **Stack syntax (Zod, Prisma, Swagger, logging) → see `.windsurf/skills/BACKEND-NEST/SKILL.md`. Ignore its UseCase / CQRS sections — they apply to ENTERPRISE modules only.**
> **Security baseline → see `.windsurf/skills/OWASP/SKILL.md`. SIMPLE modules still apply: deny-by-default guards (`JwtAuthGuard` + `CaslGuard`), Zod v4 validation at the edge, Prisma parameterized queries (OWASP #3), `ParseUUIDPipe` on `:id` params (API #1 BOLA), pagination caps on lists (API #4).**
...
> **No `domain/`, no `application/`, no `infrastructure/` folders. No `*.command.ts`, no `*.handler.ts`, no `CommandBus`/`QueryBus`, no `*.domain-event.ts`.** Those belong to `.windsurf/skills/ARCHITECTURE-ENTERPRISE/SKILL.md` and appear only after an upgrade trigger.
...
> ⚠️ `bulkDelete` / `bulkRestore` / `restore` / soft-delete repository methods do NOT belong in a SIMPLE module. The moment the UI needs them, upgrade to `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md`.

### `{module}.service.ts`
Orchestration only. Injects the repository, `LoggerService`, `ClsService`. Applies the DRY patterns from `.windsurf/skills/BACKEND-NEST-PATTERNS/SKILL.md`:
...
If your module needs **any** of the following, stop and switch to `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md` — the patterns below all live there, not here:
...
> SIMPLE modules consume only the minimum cross-cutting concerns. See `.windsurf/skills/ARCHITECTURE-DECISION-GUIDE.md` for the full `shared/` tree.
...
Escalate to `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md` when ANY of the following becomes true:
...
Escalate to `.windsurf/skills/ARCHITECTURE-ENTERPRISE/SKILL.md` when:
