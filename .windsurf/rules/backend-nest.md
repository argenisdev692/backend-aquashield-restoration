...
- **Architecture decision:** ALWAYS consult `.windsurf/skills/ARCHITECTURE-DECISION-GUIDE.md` first to pick the tier.
- **Architecture (simple):** Follow `.windsurf/skills/ARCHITECTURE-SIMPLE/SKILL.md` for lookup/config modules (≤5 fields, validate+save, no cache/audit/exports).
- **Architecture (default):** Follow `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md` for CRUDs with business logic, cache, audit, exports, bulk operations — the most common tier.
- **Architecture (upgrade):** Follow `.windsurf/skills/ARCHITECTURE-ENTERPRISE/SKILL.md` ONLY for modules that meet upgrade triggers (domain events, cross-context ACL, state machines, invariants beyond "validate + save").
...
| New module (ANY architecture)                  | `.windsurf/skills/ARCHITECTURE-DECISION-GUIDE.md` — FIRST, then the selected architecture skill |
| New module (simple lookups/configs)             | `.windsurf/skills/ARCHITECTURE-SIMPLE/SKILL.md`         |
| New module (CRUDs with business logic)          | `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md`         |
| New module (complex bounded contexts)           | `.windsurf/skills/ARCHITECTURE-ENTERPRISE/SKILL.md`       |
...
| Audit agent / code review                       | `.windsurf/workflows/BACKEND-AUDIT-NEST.md`                  |
...
- **SIMPLE — lookups/configs:** Service/Repository, 8–10 files, no cache/audit/exports/bulk. Full layout in `.windsurf/skills/ARCHITECTURE-SIMPLE/SKILL.md`.
- **DEFAULT — CRUDs with business logic:** Service/Repository with the Canonical Mutation Pattern (tx + audit + cache), bulk delete/restore, soft-delete visibility, exports. 10–12 files. Full layout and contracts live in `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md`.
- **UPGRADE — Hex/DDD + UseCase (CQRS optional):** allowed ONLY when an Upgrade Trigger is met. Upgrade triggers live in `.windsurf/skills/ARCHITECTURE-DECISION-GUIDE.md`. Full Hex/DDD layout, ports, mappers, and UseCase structure live in `.windsurf/skills/ARCHITECTURE-ENTERPRISE/SKILL.md`. `CommandBus`/`QueryBus` are opt-in per bounded context.
...
- Full spec: `.windsurf/skills/ARCHITECTURE-DEFAULT/SKILL.md` § "Users & Auth response shape — roles + permissions" and `.windsurf/skills/ARCHITECTURE-ENTERPRISE/SKILL.md` § "Users & Auth response shape — roles + permissions (Hex/DDD)".
